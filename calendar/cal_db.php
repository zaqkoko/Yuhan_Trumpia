<?php
  ...
  $d_sql = "SELECT receiver, send_message, send_time, send_type, user_id FROM sms WHERE user_id = '$id' AND send_type = 2";
  // $conn으로 연결된 객체에  $sql쿼리문을 실행시키는 함수.
  $d_result = mysqli_query($conn, $d_sql);

  $t_sql = "SELECT receiver, send_message, send_time, send_type, user_id FROM sms WHERE user_id = '$id' AND send_type = 1";
  // $conn으로 연결된 객체에  $sql쿼리문을 실행시키는 함수.
